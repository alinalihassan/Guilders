...
const selectAssetSchema = createSelectSchema(asset);
const insertAssetSchema = createInsertSchema(asset);
...
async function buildAssetHierarchy(
  ...
  for (const ast of assets) {
    let institutionConnectionData: Static<typeof InstitutionConnectionSchema> | null = null;
    
    if (ast.institution_connection_id) {
      institutionConnectionData = await fetchInstitutionConnection(ast.institution_connection_id);
    }
...
export const assetRoutes = new Elysia({ prefix: '/asset' })
  ...
  .get("", async ({ user }) => {
    ...
    detail: {
      ...
      description: "Retrieve all assets for the authenticated user with hierarchical structure",
      tags: ["Assets"]
    }
  })
  .post("", async ({ body, user }) => {
    ...
    detail: {
      ...
      description: "Create a new asset with auto-calculated type from subtype",
      tags: ["Assets"]
    }
  })
  .get("/:id", async ({ params, user }) => {
    ...
    if (assetData.institution_connection_id) {
      institutionConnectionData = await fetchInstitutionConnection(assetData.institution_connection_id);
    }
    ...
    const childrenWithData = await Promise.all(
      children.map(async (child) => {
        ...
        if (child.institution_connection_id) {
          childIc = await fetchInstitutionConnection(child.institution_connection_id);
        }
        return {
          ...child,
          children: [],
  ...
  .put("/:id", async ({ params, body, user }) => {
    ...
    detail: {
      ...
      description: "Update an asset with automatic type recalculation if subtype changed",
      tags: ["Assets"]
    }
  })
