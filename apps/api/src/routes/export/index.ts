...
import { documented, jsonError } from "../../lib/http";
...
export const exportRoutes = new Hono<AuthEnv>().use(requireAuth).get(
  "/",
  documented({
    tags: ["Export"],
    hide: true,
  ...
  async (c) => {
    ...
    if (documents.length > MAX_DOCUMENTS) {
      return jsonError(
        c,
        413,
        `Export limit exceeded: maximum ${MAX_DOCUMENTS} documents. You have ${documents.length}.`,
      );
    }
    ...
    const zippable: Record<string, Uint8Array> = {
      ...
      "transactions.json": toU8(
        JSON.stringify(
          transactions.map((tx) => transactionForExport(tx as unknown as Record<string, unknown>)),
          null,
          2,
        ),
      ),
    ...
    for (const doc of documents) {
      ...
      if (totalDocumentBytes + bytes.length >= MAX_TOTAL_DOCUMENT_BYTES) {
        return jsonError(
          c,
          413,
          `Export limit exceeded: total document size exceeds ${MAX_TOTAL_DOCUMENT_BYTES / 1024 / 1024} MB.`,
        );
      }
