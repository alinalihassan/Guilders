  ...
  return (
    <section className="relative w-full overflow-hidden pb-20 pt-32 md:pb-32 md:pt-40">
      ...
      <div className="container relative z-10 mx-auto px-4 md:px-6 flex flex-col items-center">
        ...
        >
          <Badge variant="outline" className="text-sm py-2 px-4 bg-secondary/50 backdrop-blur-md border-primary/20 rounded-full">
            ...
            <span className="text-muted-foreground">Open Source Personal Finance</span>
          </Badge>
          ...
          <p className="max-w-2xl mx-auto text-xl text-muted-foreground/80 leading-relaxed">
            Track synced accounts, manage manual assets, and get actionable insights from your personal AI analyst. Self-hosted or cloud-managed—your data stays yours.
          </p>
        ...
        >
          <div className="rounded-2xl border border-secondary/50 bg-background/40 backdrop-blur-2xl shadow-2xl overflow-hidden ring-1 ring-white/5 relative transform-gpu rotate-x-[2deg] hover:rotate-x-0 transition-transform duration-700 ease-out">
             ...
             <div className="flex items-center px-4 py-3 border-b border-border/40 bg-background/50 backdrop-blur-md">
               ...
               <div className="mx-auto text-xs font-medium text-muted-foreground/60 tracking-wider uppercase">Guilders Dashboard</div>
             </div>
             ...
             <div className="p-4 md:p-8 grid grid-cols-1 md:grid-cols-12 gap-6 relative z-10">
                ...
                <div className="md:col-span-8 flex flex-col gap-6">
                  ...
                  <div className="bg-secondary/20 rounded-2xl border border-border/50 p-6 relative overflow-hidden group">
                     <div className="absolute top-0 right-0 p-6">
                       <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20">+12.5% All Time</Badge>
                     </div>
                     <div className="text-muted-foreground font-medium mb-1 tracking-tight">Total Net Worth</div>
                     <div className="text-5xl font-bold text-foreground tracking-tight">$124,592.00</div>
                     <div className="mt-8 h-40 w-full relative">
                        ...
                        <svg className="w-full h-full text-primary drop-shadow-[0_0_8px_rgba(20,184,100,0.8)]" viewBox="0 0 100 100" preserveAspectRatio="none">
                          <title>Net worth chart</title>
                          <path d="M0,80 Q10,75 20,60 T40,70 T60,30 T80,40 T100,10" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                     </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-secondary/20 rounded-2xl border border-border/50 p-5">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="text-muted-foreground font-medium text-sm">Income</div>
                      </div>
                      <div className="text-2xl font-bold text-foreground">+$8,450.00</div>
                    </div>
                    <div className="bg-secondary/20 rounded-2xl border border-border/50 p-5">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="text-muted-foreground font-medium text-sm">Expenses</div>
                      </div>
                      <div className="text-2xl font-bold text-foreground">-$3,120.50</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="md:col-span-4 bg-secondary/20 rounded-2xl border border-border/50 p-6 flex flex-col">
                   <div className="font-semibold text-lg mb-4 tracking-tight">Recent Activity</div>
                   <div className="flex flex-col gap-4 flex-1">
                     {[
                       { icon: "🥑", name: "Whole Foods", amount: "-$142.81", date: "Today, 2:45 PM", category: "Groceries" },
                       { icon: "🍿", name: "Netflix", amount: "-$15.49", date: "Yesterday", category: "Entertainment" },
                       { icon: "💻", name: "AWS", amount: "-$42.00", date: "Yesterday", category: "Software" },
                       { icon: "🏠", name: "Rent", amount: "-$2,100.00", date: "May 1", category: "Housing" },
                       { icon: "💰", name: "Salary", amount: "+$4,225.00", date: "May 1", category: "Income", positive: true },
                     ].map((tx, i) => (
                       <div key={i} className="flex justify-between items-center group cursor-pointer">
                         <div className="flex items-center gap-3">
                           <div className="w-10 h-10 rounded-full bg-background/80 border border-border/50 shadow-sm flex items-center justify-center text-lg group-hover:scale-110 transition-transform">{tx.icon}</div>
                           <div>
                             <div className="text-sm font-medium text-foreground">{tx.name}</div>
                             <div className="text-xs text-muted-foreground">{tx.category}</div>
                           </div>
                         </div>
                         <div className={`text-sm font-semibold ${tx.positive ? 'text-primary' : 'text-foreground'}`}>{tx.amount}</div>
                       </div>
                     ))}
                   </div>
                </div>
