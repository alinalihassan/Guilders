...
function verifyWebhookSignature(rawBody: string, signatureHeader: string | null): boolean {
  ...
  const received = signatureHeader.slice(7); // "sha256=".length
...
### Web Crypto (Cloudflare Workers, browser, Node 19+)
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (!(await verifyWebhookSignature(rawBody, signature, env.GUILDERS_WEBHOOK_SECRET))) {
