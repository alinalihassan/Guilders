...
export function FinanceResponse({ type, data, colors }: FinanceResponseProps) {
  ...
  if (type === "spending_chart") {
    ...
    const total = categories.reduce((sum: number, cat: { amount: number }) => sum + cat.amount, 0);
    ...
    return (
      ...
      >
        ...
        <View style={{ gap: Spacing.two }}>
          {categories.map(
            (category: { name: string; amount: number; color: string }, index: number) => {
  ...
  if (type === "account_breakdown") {
    ...
    return (
      ...
      >
        ...
        <View style={{ gap: Spacing.two }}>
          {accounts.map((account: { name: string; value: number; type: string }, index: number) => (
            <View
